...
// constructor
	...
	<label for="<?php echo $this->get_field_id('action_text'); ?>"><?php _e('Enter your call to action.', 'bldr'); ?></label>
	...
	<label for="<?php echo $this->get_field_id('action_btn_link'); ?>"><?php _e('Link for the button', 'bldr'); ?></label>
	...
	<label for="<?php echo $this->get_field_id('action_btn_text'); ?>"><?php _e('Title for the button', 'bldr'); ?></label>
    ...
    <?php
        ...
        if ( $image_upload != '' ) :
           ...
           echo '<p><img class="custom_media_image" src="' . $image_upload . '" style="max-width:100px;" /></p>';

        endif;

    ?>

    <p><label for="<?php echo $this->get_field_id('image_upload'); ?>"><?php _e('Make your Call-to-Action background an image (Use a picture that is 1200px x 800px for best results). IMPORTANT - Go to Edit Row > Theme > Background image and paste image URL to populate. Delete the URL below after you have copied it.', 'bldr'); ?></label></p>

    <p><input type="button" class="button button-primary custom_media_button" id="custom_media_button" name="<?php echo $this->get_field_name('image_upload'); ?>" value="Upload Image" style="margin-top:5px;" /></p>

	<p><input class="widefat custom_media_url" id="<?php echo $this->get_field_id( 'image_upload' ); ?>" name="<?php echo $this->get_field_name( 'image_upload' ); ?>" type="text" value="<?php echo $image_upload; ?>" size="3" /></p>	
		...
		<section id="home-cta" class="action-area">
            ...
            <?php if ($action_btn_link !='') : ?>
					...
					<a href="<?php echo esc_url($action_btn_link); ?>" class="call-to-action">
						...
						<button class="animate-plus" data-animations="fadeIn" data-animation-when-visible="true" data-animation-delay="0.2s"><?php echo esc_html($action_btn_text); ?></button>
			...
			<style type="text/css">
				...
				.action-area {
					...
					background: url(<?php echo $image_upload; ?>) no-repeat;
