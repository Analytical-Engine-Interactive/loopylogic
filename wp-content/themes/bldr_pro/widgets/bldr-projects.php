...
class bldr_projects extends WP_Widget {



// constructor

    function bldr_projects() {

		$widget_ops = array('classname' => 'bldr_projects_widget', 'description' => __( 'With this widget you can display all those projects you are proud of.', 'bldr') );

        parent::__construct(false, $name = __('Home Page Projects', 'bldr'), $widget_ops);

		$this->alt_option_name = 'bldr_projects_widget';
	...
	<p><?php _e('In order to display this widget, you must first add some Projects custom post types.', 'bldr'); ?></p>
	...
	<p><label for="<?php echo $this->get_field_id( 'number' ); ?>"><?php _e( 'Number of projects to show (-1 shows all):', 'bldr' ); ?></label>
    ...
    <p><label for="<?php echo $this->get_field_id('see_all'); ?>"><?php _e('<em>Optional:</em> Enter the URL for your Projects page.', 'bldr'); ?></label>
	...
	function update($new_instance, $old_instance) {
		...
		if ( isset($alloptions['bldr_projects']) )

			delete_option('bldr_projects');		  
	...
	function flush_widget_cache() {
		...
		wp_cache_delete('bldr_projects', 'widget');
	...
	function widget($args, $instance) {
		...
		if ( ! $this->is_preview() ) {
			...
			$cache = wp_cache_get( 'bldr_projects', 'widget' );
		...
		$title = ( ! empty( $instance['title'] ) ) ? $instance['title'] : __( 'Projects', 'bldr' ); 
...
?>
		...
		<section id="home-projects" class="projects"> 
            ...
            <div class="grid grid-pad overflow">		
                ...
                <div class="col-1-3 tri-clear animate-plus" data-animations="fadeIn" data-animation-when-visible="true" data-animation-delay="0.3s">  
	...
	<?php
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'bldr_projects', $cache, 'widget' );
